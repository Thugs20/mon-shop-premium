...
const Navbar = () => {
  ...
  <div className="flex items-center justify-between">
    ...
    {/* GAUCHE : Bloc Burger + Logo (Largeur minimum garantie) */}
    ...
    {/* DROITE : Actions (Largeur ajustée pour ne pas déborder) */}
    <div className="flex items-center justify-end gap-1 sm:gap-3 shrink-0">
      ...
      <button className="hidden sm:block text-slate-300 hover:text-white p-1.5">
        <User size={20} />
      </button>
  ...
  {isOpen && (
    <div className="md:hidden mt-4 pt-4 border-t border-white/10 flex flex-col gap-4 text-slate-300 font-medium pb-2">
      ...
      <Link href="/favoris" onClick={() => setIsOpen(false)} className="text-[#38bdf8]">Mes Favoris ({favorites.length})</Link>
    </div>
  )}
</nav>
      </header>
